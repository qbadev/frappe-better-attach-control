(() => {
  ...
  frappe.ui.form.ControlAttach = frappe.ui.form.ControlAttach.extend({
    ...
    toggle_reload_button: function() {
      if (!this.$value)
        return;
      var show = this._allow_reload && this.file_uploader && this.file_uploader.uploader.files && this.file_uploader.uploader.files.length > 0;
    ...
    _setup_popover: function() {
      if (this._popover_ready)
        return;
      var me = this;
      this.$value.find(".attached-file-link").first().popover({
        trigger: "hover",
        placement: "top",
        content: function() {
          var file = !me._images_only ? me._files[me._files.length - 1] : null, url = file ? file.file_url : me.value;
